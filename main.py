...
class Game:
     def __init__(self):
         ...
         linkBackGround = 'Space-Invader\\data\\background.jpg'  # Đường dẫn ảnh background
         self.linkEnemy = 'Space-Invader\\data\\enemy.png'  # Đường dẫn ảnh Enemy
         self.linkEnemyKilled = 'Space-Invader\\data\\enemykilled.png'
         self.linkPlanes = 'Space-Invader\\data\\planes.png'  # Đường dẫn ảnh Planes
         self.linkChicken = 'Space-Invader\\data\\chicken.png'
         ...
         self.music("Space-Invader\\data\\musictheme.wav")
     ...
     def bullet(self):
         for count, i in enumerate(self.listBullet):
             ...
             self.image_draw('Space-Invader\\data\\bullet.png', xBullet,
     ...
     def restart(self):
                ...
                newGame = False
                pygame.mixer.stop()
                self.music("Space-Invader\\data\\gameover.wav")
                while(True):
                     ...
                     if(newGame == True):  # Thoát vòng while để vào game mới
                         break
                     self.text(self.xScreen/2 - 200, self.yScreen/2-100,
                                     "GAME OVER", 50 , "Space-Invader\\data\\font\\ARCADE_N.TTF",WHITE)  # In Thông báo thua
                     self.text(self.xScreen/2 - 50, self.yScreen/2, "Scores:{}".format(
                         self.scores), 20, "Space-Invader\\data\\font\\ARCADE_R.TTF",WHITE)  # In điểm
                 ...
                 if event.type == pygame.KEYDOWN:  # sự kiện có phím nhấn xuống
                     ...
                     if event.key == pygame.K_SPACE:
                         if len(self.listBullet) < self.numberBullet:
                             self.music("Space-Invader\\data\\laser.wav")
...
#            #  nếu số lượng Gà ít hơn self.numberChicken thì tạo thêm
             ...
             if self.YGameOver > self.yScreen-50:  # Nếu Enemy về gần đích
                 self.restart()
             self.text(10, 10, "Scores:{}".format(self.scores), 15,"Space-Invader\\data\\font\\ARCADE_R.TTF",WHITE)
             #self.show_score(self.xScreen - 200, 20, "duyduysysy@gmail.com", 15)
             #self.image_draw("Space-Invader\\data\\codelearn-logo.png",
